...
pub fn print_elf_info(
	...
	if offset <= 2 {
		window.mvaddstr(
			2 - offset,
			60,
			&format!("&progs: 0x{:08X}", info.prog_head),
		);
		window.attrset(ColorPair(0));
		window.printw(" ");
		window.attrset(ColorPair(6));
	...
	if offset <= 3 {
		window.mvaddstr(
			3 - offset,
			60,
			&format!("Flags: 0x{:04X}", info.flags),
		);
		window.attrset(ColorPair(0));
		window.printw(" ");
		window.attrset(ColorPair(2));
	...
	for tab in &info.strtabs {
		for i in tab.offset / 16..((tab.offset + tab.size) / 16 + 1) {
			if i as i32 >= offset
				&& (i as i32) - offset < window.get_max_y() - 1
			{
				...
				for k in 0..16 {
					...
					window.printw(&format!(
						"{}",
						if index < buffer.len()
							&& buffer[index] >= 32 && buffer[index] < 127
						{
							buffer[index] as char
						} else {
							'.'
						}
					));
	...
	for i in 0..(info.progs.len() as i32) {
		...
		let base = ((info.prog_head + ((i as u64) * (info.prog_size as u64)))
			/ 16) as i32 - offset;
		...
		if base >= 0 && base < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				...
				match head.typ {
					...
					0x70000000..=0x7FFFFFFF => "Proccesor specific",
					_ => "Unknown type",
				},
			);
			window.attrset(ColorPair(0));
		...
		if base + 1 >= 0 && base + 1 < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				base + 1,
				60,
				&format!("Virt Addr: 0x{:08X}", head.virt_addr),
			);
			window.attrset(ColorPair(0));
			window.printw(" ");
			window.attrset(ColorPair(6));
		...
		if base + 2 >= 0 && base + 2 < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				base + 2,
				60,
				&format!("Size in mem: 0x{:08X}", head.mem_size),
			);
			window.attrset(ColorPair(0));
			window.printw(" ");
			window.attrset(ColorPair(3));
	...
	for i in 0..(info.sects.len() as i32) {
		...
		let base = ((info.sect_head + ((i as u64) * (info.sect_size as u64)))
			/ 16) as i32 - offset;
		...
		if base >= 0 && base < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				base,
				60,
				&format!("Name: {} ({})", &head.name_str, head.name),
			);
			window.attrset(ColorPair(0));
			window.printw(" ");
			window.attrset(ColorPair(2));
			window.printw(match head.typ {
				0 => "NULL",
				1 => "PROGBITS",
				2 => "SYMTAB",
				3 => "STRTAB",
		...
		if base + 2 >= 0 && base + 2 < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				base + 2,
				60,
				&format!("Size in file: 0x{:08X}", head.file_size),
			);
			window.attrset(ColorPair(0));
			window.printw(" ");
			window.attrset(ColorPair(1));
		...
		if base + 3 >= 0 && base + 3 < window.get_max_y() - 1 {
			...
			window.mvaddstr(
				base + 3,
				60,
				&format!("Alignment: 0x{:08X}", head.align),
			);
			window.attrset(ColorPair(0));
			window.printw(" ");
			window.attrset(ColorPair(4));
	...
	for tab in &info.reltabs {
		for i in 0..tab.rels.len() {
			let base = ((tab.offset + (i as u64) * (tab.entry_size as u64))
				/ 16) as i32 - offset;
			if base >= 0 && base < window.get_max_y() - 1 {
				window.attrset(ColorPair(1));
				window.mvaddstr(
					base,
					60,
					&format!("Offset: 0x{:04x}", tab.rels[i].offset),
				);
				window.attrset(ColorPair(0));
				window.printw(" ");
				if info.arch == 0x3E {
